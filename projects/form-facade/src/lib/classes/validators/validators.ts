...
export type ExtendedValidatorFn<T, K extends keyof T> = (ctrl: AbstractControl, values: Pick<T, K>, facade: FormFacade<T>) => ValidationErrors | null;
...
export function composeValidators(validators: ValidatorFn[]): ValidatorFn
{
    ...
    if (dependents)
        return makeDependentValidator<any>()(dependents, fn);
    else
        return fn;
}
...
): ValidatorFn
{
    ...
    if (dependents)
        return makeDependentValidator<any>()(dependents, warningValidatorFn);
    else
        return warningValidatorFn;
}
 ...
 * @returns the decorated validator function
 */
export function makeDependentValidator<T>(): <K extends keyof T>(prop: K, validator: ExtendedValidatorFn<T, K>) => ValidatorFn;
export function makeDependentValidator<T>(): <K extends keyof T, K2 extends Exclude<keyof T, K>>(prop: [K, K2], validator: ExtendedValidatorFn<T, K | K2>) => ValidatorFn;
export function makeDependentValidator<T>(): <K extends keyof T, K2 extends Exclude<keyof T, K>, K3 extends Exclude<keyof T, K | K2>>(prop: [K, K2, K3], validator: ExtendedValidatorFn<T, K | K2 | K3>) => ValidatorFn;
export function makeDependentValidator<T>(): (prop: (keyof T)[], validator: ExtendedValidatorFn<T, keyof T>) => ValidatorFn;
export function makeDependentValidator<T>(): (prop: keyof T | (keyof T)[], exValidator: ExtendedValidatorFn<T, keyof T>) => ValidatorFn
{
